...
static inline void daxpy_cache(uint64_t n, lvector double a, lvector double *x, lvector double *y)
{
...
static inline void daxpy_single_am(uint64_t n, double a, double *x, double *y)
{
...
static inline void daxpy_single_mem(uint64_t n, double a, double *x, double *y)
{
    uint64_t n_cache = M_MT_CacheNum * M_MT_VecLen;
    // malloc cache memory
    lvector double *x_cache = (lvector double *)vector_malloc(n_cache * sizeof(double));
...
__global__ void daxpy_kernel(uint64_t n, double a, double *x, double *y)
{
    ...
    daxpy_single_am(n_p, a, x + offset, y + offset);
}

__global__ void simd_test(uint64_t n, double a, double *x, double *y)
{
    const int len = 3;
    lvector unsigned long* buffer = (lvector unsigned long*)vector_malloc(len * 16 * sizeof(unsigned long));
    for (int i = 0; i < len; i++)
    {
        lvector unsigned long tmp = vec_svbcast(i + 1);
        vec_st(tmp, 0, buffer + i);

    }
    lvector unsigned long x_vec = vec_ld(1, buffer);
    printVec64l(x_vec);
}