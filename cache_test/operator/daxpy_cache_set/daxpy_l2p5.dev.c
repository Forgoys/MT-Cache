...
static inline void daxpy_cache(uint64_t n, double a, double *x, double *y)
{
...
static inline void initJobs(Job *jobs, uint64_t num, double *x, double *y)
{
...
static inline void Job_set(Job *job, uint64_t offset, uint64_t n_actual)
{
    job->offset = offset;
    job->n_actual = n_actual;
}
static inline void Job_setCache(Job *job, double *x_cache, double *y_cache)
{
    job->x_cache = x_cache;
    job->y_cache = y_cache;
}
