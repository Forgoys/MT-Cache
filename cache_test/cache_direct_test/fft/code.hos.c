...
int main(int argc, char **argv)
{
...

    {
        ...
        sprintf(filename, "%d*%d fft.txt", mul,base);
    ...
    if (file == NULL) {
        ...
        printf("*==========================*\n\n");
        int sum, cets = 1, lines = 1;
        int sum_begin = 9, sum_end = 15;
            for (sum = sum_begin; sum <= sum_end; sum++)
        {
                for (cets = 1; cets <= sum - 3; cets++)
                {
                    ...
                    speed_up = (baseline_time_sum - test_time_sum) / baseline_time_sum * 100; // Convert to percentage
                    printf("test time:(cets:%d,lines:%d): %lf seconds,speed up(百分数) : %.2f%%\n",cets,lines, test_time_sum,speed_up);
                    fprintf(file,"test time:(cets:%d,lines:%d): %lf seconds,speed up(百分数) : %.2f%%\n",cets,lines, test_time_sum,speed_up);
                }
        }
...
/*
double dsp_test(int clusterId,uint64_t n, double *arr, int cets, int lines){
    uint64_t args[4];
    args[0] = n;
    args[1] = (uint64_t)arr;
    args[2] = cets;
...
int main(int argc, char** argv)
{
        ...
        for (sum = sum_begin; sum <= sum_end; sum++)
    {
        ...
        {
            lines = sum - cets;
            dev_time = dsp_test(clusterId,n, arr,cets, lines);
