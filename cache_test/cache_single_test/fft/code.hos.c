...
int main(int argc, char **argv)
{
    ...
    for (int mul = 1; mul < 4; mul = mul * 2)
    {
        ...
        printf("*==========================*\n\n");
        int sets = 0, lines = 4;  // 单缓冲区只有一个lines参数
        for (; lines <= 9; lines++)
        {
            ...
            printf("test time:(sets:%d,lines:%d): %lf seconds,speed up(百分数) : %.2f%%\n", sets, lines, test_time_sum, speed_up);
            fprintf(file, "test time:(sets:%d,lines:%d): %lf seconds,speed up(百分数) : %.2f%%\n", sets, lines, test_time_sum, speed_up);
        }
